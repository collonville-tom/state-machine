...
import org.tc.osgi.bundle.solve.transformation.module.service.CollectionUtilsServiceProxy;
...
public class LpSatisfyOnIState extends AbstractSatisfy {
    ...
    private void analyseState(final IState state) {
        ...
        final IState s = CollectionUtilsServiceProxy.getInstance().extract(getTransitionStructure().getStates(), new IPredicate<IState>() {
            @Override
    ...
    public IPa getPa(final String pa) {
        ...
        return CollectionUtilsServiceProxy.getInstance().extract(getPropertySet(), new IPredicate<IPa>() {
